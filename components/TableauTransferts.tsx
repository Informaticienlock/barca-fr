...
const ARRIVEES: Transfert[] = [
  {
    ...
    clubDepart: 'RB Leipzig', clubArrivee: 'FC Barcelone',
    type: 'Transfert', montant: '55 M€',
    statut: 'Officiel', date: '17 mai 2026',
  },
  {
    id: 2,
    ...
    clubDepart: 'Bayern Munich', clubArrivee: 'FC Barcelone',
    type: 'Fin de contrat', montant: '0 M€',
    statut: 'Accord trouvé', date: '16 mai 2026',
  },
  {
    id: 3,
    joueur: 'Jonathan Tah',
    ...
    clubDepart: 'Bayer Leverkusen', clubArrivee: 'FC Barcelone',
    type: 'Fin de contrat', montant: '0 M€',
    statut: 'En négociation', date: '15 mai 2026',
  },
  {
    id: 4,
    ...
    clubDepart: 'Athletic Club', clubArrivee: 'FC Barcelone',
    type: 'Transfert', montant: '58 M€',
    statut: 'En négociation', date: '14 mai 2026',
  },
]
...
const DEPARTS: Transfert[] = [
  {
    id: 5,
    joueur: 'Sergi Roberto',
    age: 32, nationalite: '🇪🇸',
    poste: 'Milieu / Latéral', positionCode: 'MF',
    clubDepart: 'FC Barcelone', clubArrivee: 'Non défini',
    type: 'Fin de contrat', montant: '0 M€',
    statut: 'Officiel', date: '1 juin 2026',
  },
  {
    id: 6,
    joueur: 'Vitor Roque',
    ...
    clubDepart: 'FC Barcelone', clubArrivee: 'Betis (prêt)',
    type: 'Prêt', montant: '0 M€',
    statut: 'Officiel', date: '10 mai 2026',
  },
  {
    id: 7,
    joueur: 'Ansu Fati',
    ...
    poste: 'Ailier Gauche', positionCode: 'LW',
    clubDepart: 'FC Barcelone', clubArrivee: 'Non défini',
    type: 'Transfert', montant: '25 M€',
    statut: 'En négociation', date: '12 mai 2026',
  },
  {
    id: 8,
    joueur: 'Clément Lenglet',
    age: 29, nationalite: '🇫🇷',
    poste: 'Défenseur Central', positionCode: 'CB',
    clubDepart: 'FC Barcelone', clubArrivee: 'Non défini',
    type: 'Fin de contrat', montant: '0 M€',
...
function LigneTransfert({ t, isArrivee }: { t: Transfert; isArrivee: boolean }) {
  ...
  return (
    <div style={{
      display: 'grid',
      gridTemplateColumns: '40px 1fr 1fr 100px 80px 90px',
      alignItems: 'center',
      gap: '12px',
      padding: '12px 16px',
    ...
    >
      ...
      <div style={{
        ...
        background: isArrivee
          ? 'linear-gradient(135deg,rgba(0,77,152,0.5),rgba(0,77,152,0.2))'
          : 'linear-gradient(135deg,rgba(165,0,68,0.5),rgba(165,0,68,0.2))',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontSize: '11px', fontWeight: 900, color: 'rgba(255,255,255,0.7)',
        fontFamily: 'DM Mono,monospace',
        border: `1px solid ${isArrivee ? 'rgba(0,77,152,0.4)' : 'rgba(165,0,68,0.4)'}`,
      }}>
        {initiales}
      </div>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
          <span style={{ fontSize: '10px', fontFamily: 'DM Mono,monospace', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.05)', padding: '1px 5px', borderRadius: '3px' }}>
            {t.positionCode}
          </span>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span style={{ fontSize: '10px', color: isArrivee ? '#4ade80' : '#f87171', flexShrink: 0 }}>
            {isArrivee ? '→' : '←'}
          </span>
          <span style={{ fontSize: '11px', color: 'white', fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {t.clubArrivee}
          </span>
        </div>
        <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'DM Mono,monospace', marginTop: '2px' }}>
          {t.date}
        </div>
      </div>
      ...
      {/* Montant */}
      <div style={{
        fontFamily: 'DM Mono,monospace', fontSize: '13px', fontWeight: 900,
        color: t.montant === '0 M€' ? 'var(--text-muted)' : 'var(--barca-gold)',
        textAlign: 'right',
      }}>
        {t.montant}
      </div>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '10px' }}>
        <div style={{
          ...
          background: isArrivee ? 'linear-gradient(135deg,rgba(0,77,152,0.5),rgba(0,77,152,0.2))' : 'linear-gradient(135deg,rgba(165,0,68,0.5),rgba(165,0,68,0.2))',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: '11px', fontWeight: 900, color: 'rgba(255,255,255,0.7)', fontFamily: 'DM Mono,monospace',
          border: `1px solid ${isArrivee ? 'rgba(0,77,152,0.4)' : 'rgba(165,0,68,0.4)'}`,
        }}>{initiales}</div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px', marginBottom: '4px' }}>
            ...
            <span style={{
              fontSize: '10px', fontFamily: 'DM Mono,monospace', fontWeight: 700,
              color: statut.color, background: statut.bg, border: `1px solid ${statut.border}`,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '10px', fontFamily: 'DM Mono,monospace', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '3px' }}>
              {t.positionCode} · {t.age} ans
            </span>
            <span style={{ fontSize: '10px', fontFamily: 'DM Mono,monospace', fontWeight: 700, color: type.color, background: type.bg, padding: '2px 6px', borderRadius: '3px' }}>
              {t.type}
            </span>
            <span style={{ fontFamily: 'DM Mono,monospace', fontSize: '12px', fontWeight: 900, color: t.montant === '0 M€' ? 'var(--text-muted)' : 'var(--barca-gold)', marginLeft: 'auto' }}>
              {t.montant}
            </span>
          </div>
        </div>
...
export default function TableauTransferts() {
  ...
  return (
    <div style={{ background: 'var(--barca-card)', border: '1px solid var(--barca-border)', borderRadius: '8px', overflow: 'hidden' }}>
      ...
      {/* Header */}
      <div style={{
        background: 'linear-gradient(135deg,rgba(0,77,152,0.15),rgba(165,0,68,0.15))',
        ...
        <div style={{ fontFamily: 'DM Mono,monospace', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.1em', marginBottom: '4px' }}>
          TABLEAU DES TRANSFERTS · MERCATO ÉTÉ 2026
        </div>
        ...
        {([['arrivees', '⬆ ARRIVÉES', ARRIVEES.length], ['departs', '⬇ DÉPARTS', DEPARTS.length]] as const).map(([key, label, count]) => (
          <button key={key} onClick={() => setOnglet(key)} style={{
            ...
            background: onglet === key
              ? key === 'arrivees' ? 'rgba(0,77,152,0.12)' : 'rgba(165,0,68,0.12)'
              : 'transparent',
            color: onglet === key ? 'white' : 'var(--text-muted)',
            fontFamily: 'DM Mono,monospace', fontSize: '11px', fontWeight: 700,
            letterSpacing: '0.06em',
            borderBottom: onglet === key
              ? `2px solid ${key === 'arrivees' ? 'var(--barca-blue)' : 'var(--barca-red)'}`
            ...
            <span style={{
              ...
              background: onglet === key ? (key === 'arrivees' ? 'rgba(0,77,152,0.3)' : 'rgba(165,0,68,0.3)') : 'rgba(255,255,255,0.08)',
              color: onglet === key ? 'white' : 'var(--text-muted)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', borderBottom: '1px solid var(--barca-border)' }}>
        {[
          ...
          <div key={i} style={{
            ...
            borderRight: i < 2 ? '1px solid var(--barca-border)' : 'none',
          }}>
      ...
      <div className="tableau-desktop-header" style={{
        display: 'grid',
        gridTemplateColumns: '40px 1fr 1fr 100px 80px 90px',
      ...
      {/* Lignes — desktop */}
      <div className="tableau-desktop">
        {data.map(t => (
          <LigneTransfert key={t.id} t={t} isArrivee={onglet === 'arrivees'} />
        ))}
      </div>
      ...
      <div className="tableau-mobile">
        {data.map(t => (
          <LigneMobile key={t.id} t={t} isArrivee={onglet === 'arrivees'} />
        ))}
      </div>

      {/* Footer */}
      <div style={{
        padding: '12px 16px', borderTop: '1px solid var(--barca-border)',
      ...
      <style>{`
        ...
        .tableau-desktop-header { display: grid; }
        @media (max-width: 680px) {
          .tableau-desktop        { display: none;  }
          .tableau-desktop-header { display: none;  }
